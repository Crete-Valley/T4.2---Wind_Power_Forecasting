...
def train_lstm(x_train, x_test, y_train, y_test, i, scaler, params):
    ...
    lstm_results = pd.read_csv('results/lstm_results.csv', index_col=0)
    ...
    model.save('models/LSTM/'+file_name+'.keras')
    ...
    model = load_model('models/LSTM/'+file_name+'.keras')

    pred_test = model.predict(x_test)

    np.save('predictions/LSTM/'+file_name+'_predictions.npy', pred_test)

    preds_test = scaler.inverse_transform(pred_test)*1000
    actuals_test = scaler.inverse_transform(y_test)*1000
    ...
    if i == 1:
        lstm_results.loc[0] = ['Vestas_data_history', 'Test_forecast', mae_test, rmse_test, mape_test, r2_score_test]
    elif i == 2:
        lstm_results.loc[1] = ['Vestas_data_forecast', 'Test_forecast', mae_test, rmse_test, mape_test, r2_score_test]
    elif i == 3:
        lstm_results.loc[2] = ['Nordex_data_history', 'Test_forecast', mae_test, rmse_test, mape_test, r2_score_test]
    elif i == 4:
        lstm_results.loc[3] = ['Nordex_data_forecast', 'Test_forecast', mae_test, rmse_test, mape_test, r2_score_test]


    lstm_results.to_csv('results/lstm_results.csv')

    del pred_test, actuals_test, history, model
...
lstm_results = pd.DataFrame(columns=['Dataset', 'Predictions on :', 'MAE', 'RMSE', 'MAPE', 'R2'])
...
x_train_h_V = np.load('LSTM input/x_train_history-forecast_Vestas.npy')
y_train_h_V = np.load('LSTM input/y_train_history-forecast_Vestas.npy')
x_test_f_V = np.load('LSTM input/x_test_history-forecast_Vestas.npy')
y_test_f_V = np.load('LSTM input/y_test_history-forecast_Vestas.npy')
...
x_train_f_V = np.load('LSTM input/x_train_forecast-forecast_Vestas.npy')
y_train_f_V = np.load('LSTM input/y_train_forecast-forecast_Vestas.npy')
x_test_f_V = np.load('LSTM input/x_test_forecast-forecast_Vestas.npy')
y_test_f_V = np.load('LSTM input/y_test_forecast-forecast_Vestas.npy')
...
x_train_h_N = np.load('LSTM input/x_train_history-forecast_Nordex.npy')
y_train_h_N = np.load('LSTM input/y_train_history-forecast_Nordex.npy')
x_test_f_N = np.load('LSTM input/x_test_history-forecast_Nordex.npy')
y_test_f_N = np.load('LSTM input/y_test_history-forecast_Nordex.npy')
...
x_train_f_N = np.load('LSTM input/x_train_forecast-forecast_Nordex.npy')
y_train_f_N = np.load('LSTM input/y_train_forecast-forecast_Nordex.npy')
x_test_f_N = np.load('LSTM input/x_test_forecast-forecast_Nordex.npy')
y_test_f_N = np.load('LSTM input/y_test_forecast-forecast_Nordex.npy')
...
table = tabulate(lstm_results, headers='keys', tablefmt='fancy_grid', stralign='center', numalign='center')
