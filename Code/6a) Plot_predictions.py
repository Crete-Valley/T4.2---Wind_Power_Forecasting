...
def metrics_day_results(dataset):
    ...
    if dataset == 'LSTM':
        ...
        predictions_V_h = np.load("predictions/LSTM/LSTM_h_V_predictions.npy")
        predictions_V_f = np.load("predictions/LSTM/LSTM_f_V_predictions.npy")
        predictions_N_h = np.load("predictions/LSTM/LSTM_h_N_predictions.npy")
        predictions_N_f = np.load("predictions/LSTM/LSTM_f_N_predictions.npy")
        
        preds_list = [predictions_V_h, predictions_V_f, predictions_N_h, predictions_N_f]
        actuals_list = [y_test_lstm_h_V, y_test_lstm_f_V, y_test_lstm_h_N, y_test_lstm_f_N]
        ...
        for period in period_list:
            ...
            plt.tight_layout()
            plt.suptitle("LSTM models results {}".format(period), y=0.8)
            # Save plot
            plt.savefig('results/LSTM/lstm_results_{}.png'.format(str(period)), bbox_inches='tight', dpi=300)
            plt.show()

    if dataset == 'TFT':

        predictions_V_h = np.load("predictions/TFT/TFT_h_V_predictions.npy")
        predictions_V_f = np.load("predictions/TFT/TFT_f_V_predictions.npy")
        predictions_N_h = np.load("predictions/TFT/TFT_h_N_predictions.npy")
        predictions_N_f = np.load("predictions/TFT/TFT_f_N_predictions.npy")

        preds_list = [predictions_V_h, predictions_V_f, predictions_N_h, predictions_N_f]
        actuals_list = [test_tft_h_V, test_tft_f_V, test_tft_h_N, test_tft_f_N]
        ...
        for period in period_list:
            ...
            plt.suptitle("TFT models results {}".format(period), y=0.8)
            # Save plot
            plt.savefig('results/TFT/tft_results_{}.png'.format(str(period)), bbox_inches='tight', dpi=300)
            plt.show()
...
def Theoretical_RMSE():
    ...
    preds_list = [x_test_lstm_f_V, x_test_lstm_f_N]
    actuals_list = [y_test_lstm_f_V, y_test_lstm_f_N]
    scalers_list = [lstm_scaler_f_V, lstm_scaler_f_N]
    period = 144
    offset_list = [50,360]
    ...
    for preds,actuals,scaler,offset in zip(preds_list,actuals_list,scalers_list,offset_list):
        ...
        rmse_test = np.sqrt(mean_squared_error(actuals_unscaled[offset:offset+period], preds_unscaled[offset:offset+period]))
    ...
    for preds,actuals,scaler in zip(preds_list,actuals_list,scalers_list):
        ...
        rmse_test = np.sqrt(mean_squared_error(actuals_unscaled, preds_unscaled))
    ...
    dataset_names = ["Vestas_24", "Nordex_24", "Vestas", "Nordex"]
    ...
    plt.tight_layout()
    plt.suptitle("Theoretical RMSE {}".format(period), y=0.8)
    # Save plot
    plt.show()
...
def metrics_total_results(dataset):
    ...
    if dataset == 'LSTM':
        ...
        plt.tight_layout()
        plt.suptitle("LSTM models total results", y=0.8)
        # Save plot
        plt.savefig('results/LSTM/lstm_total_results.png', bbox_inches='tight', dpi=300)
        plt.show()
    ...
    if dataset == 'TFT':
        ...
        plt.tight_layout()
        plt.suptitle("TFT models total results", y=0.8)
...
def plot_comparative_results(dataset, period):
    ...
    if dataset == 'V':
        ...
        actuals = lstm_scaler_f_V.inverse_transform(y_test_lstm_f_V)*1000

        preds1_scaled = np.load('predictions/LSTM/LSTM_f_V_predictions.npy')
        preds2_scaled = np.load('predictions/TFT/TFT_f_V_predictions.npy')
        ...
        plt.plot(x, preds1[offset+2:offset+period+2], color='#8856a7', alpha=0.8, linewidth = 1.5, label='PI LSTM Predictions')
        plt.plot(x, preds2[offset:offset+period], color='#74c476', alpha=0.8, linewidth = 1.5, label='Proposed Method Predictions')
        plt.fill_between(x, zeros, error1, color='#810f7c', alpha=0.4, label='PI LSTM Error')
        plt.fill_between(x, zeros, error2, color='#31a354', alpha=0.4, label='Proposed Method Error')

        plt.xlabel('Time-step (10 minutes)')
        plt.ylabel('Power Consumption (kW)')
        plt.legend()
        plt.grid(True)
        plt.savefig('plots/Comparative/Vestas_plot.png', dpi=300)
        plt.show()
        ...
        actuals = lstm_scaler_f_N.inverse_transform(y_test_lstm_f_N)*1000

        preds1_scaled = np.load('predictions/LSTM/LSTM_f_N_predictions.npy')
        preds2_scaled = np.load('predictions/TFT/TFT_f_N_predictions.npy')
        ...
        plt.legend()
        plt.grid(True)
        plt.savefig('plots/Comparative/Nordex_plot.png', dpi=300)
        plt.show()
...
def error_plots(dataset, period):
    ...
    if dataset == 'V':
        ...
        offset = 50
        true_measurements_unscaled_LSTM = lstm_scaler_f_V.inverse_transform(y_test_lstm_f_V)*1000
        true_measurements_unscaled_LSTM = true_measurements_unscaled_LSTM.ravel()

        y_test = test_tft_f_V['ActivePower'][30:-24]
        ...
        offset = 360
        true_measurements_unscaled_LSTM = lstm_scaler_f_N.inverse_transform(y_test_lstm_f_N)*1000
        true_measurements_unscaled_LSTM = true_measurements_unscaled_LSTM.ravel()

        y_test = test_tft_f_N['ActivePower'][30:-24]
    ...
    plt.xticks([0, 1], ["PI LSTM Error", "Proposed Method"])
    plt.ylabel("Error Distributions")

    plt.savefig('plots/Error plots/Error_violin_plot_total_{}.png'.format(dataset), dpi=500)
    plt.show()
...
def plot_predictions(model, type, dataset, scaler, period, y_test):
    ...
    if model == 'LSTM':
        ...
        pred_scaled = np.load('predictions/LSTM/LSTM_'+type+'_'+dataset+'_predictions.npy')
        ...
        pred_scaled = np.load('predictions/TFT/TFT_'+type+'_'+dataset+'_predictions.npy')
...
Vestas_data_f = pd.read_csv('datasets/Vestas_V52_preprocessed_forecast_data.csv')
Nordex_data_f = pd.read_csv('datasets/Nordex_N117_preprocessed_forecast_data.csv')
...
x_test_lstm_f_V = split(Vestas_data_f)
x_test_lstm_f_V = x_test_lstm_f_V['TheoreticalPower']*1000
x_test_lstm_f_V = x_test_lstm_f_V[30:]

x_test_lstm_f_N = split(Nordex_data_f)
x_test_lstm_f_N = x_test_lstm_f_N['TheoreticalPower']*1000
x_test_lstm_f_N = x_test_lstm_f_N[30:]
...
plot_predictions('LSTM', 'h', 'V', lstm_scaler_h_V, 144, y_test_lstm_h_V)
plot_predictions('LSTM', 'f', 'V', lstm_scaler_f_V, 144, y_test_lstm_f_V)
plot_predictions('LSTM', 'h', 'N', lstm_scaler_h_N, 144, y_test_lstm_h_N)
plot_predictions('LSTM', 'f', 'N', lstm_scaler_f_N, 144, y_test_lstm_f_N)
...
plot_predictions('TFT', 'h', 'V', tft_scaler_h_V, 144, test_tft_h_V)
plot_predictions('TFT', 'f', 'V', tft_scaler_f_V, 144, test_tft_f_V)
plot_predictions('TFT', 'h', 'N', tft_scaler_h_N, 144, test_tft_h_N)
plot_predictions('TFT', 'f', 'N', tft_scaler_f_N, 144, test_tft_f_N)
